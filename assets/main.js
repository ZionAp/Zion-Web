...
(function() {
  ...
  // Header Scroll Effect - Optimized with requestAnimationFrame throttling
  const HeaderScroll = {
    ticking: false,
    currentScroll: 0,
    ...
    init() {
      document.querySelectorAll('.faq-item').forEach(item => {
        item.addEventListener('toggle', () => {
          // Just toggle class; animation is handled by CSS
  ...
  // Hero Animation Performance - Pause when scrolled out of view
  const HeroAnimationController = {
    init() {
      ...
      if (!heroSection || (!heroBird && !heroPattern)) return;
      
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          const isPaused = !entry.isIntersecting;
          if (heroBird) {
            heroBird.style.animationPlayState = isPaused ? 'paused' : 'running';
            heroBird.querySelectorAll('.scene-drop').forEach(drop => {
              drop.style.animationPlayState = isPaused ? 'paused' : 'running';
            });
          }
          if (heroPattern) {
            heroPattern.style.animationPlayState = isPaused ? 'paused' : 'running';
            heroPattern.querySelectorAll('.hero-piece').forEach(piece => {
              piece.style.animationPlayState = isPaused ? 'paused' : 'running';
            });
          }
        });
      }, {
